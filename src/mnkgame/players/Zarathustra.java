 ...
 * - 
 */
public class Zarathustra implements MNKPlayer {
  ...
  private Map<MNKBoard, Integer> memo;
  ...
  private int rank_board(MinimaxBoard b, Action action, int moves) {
    ...
    if(!memo.containsKey(b)){
        ...
        for(MNKCell c : b.getFreeCells()) {
          b.markCell(c);
          int rank = rank_board( (MinimaxBoard) b.clone(), action == Action.MAXIMIZE ? Action.MINIMIZE
  ...
  private void printMemo(){
    System.out.println("MEMO");
      for(Map.Entry<MNKBoard, Integer> e : memo.entrySet()){
