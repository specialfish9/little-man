...
public class GalileoGalilei implements MNKPlayer {
  ...
  private static class Board extends MNKBoard {
    ...
    private double pushCell(final MNKCellState state) {
      ...
      if (queueP1 + queueFree == K)
        return (me == MNKCellState.P1 ? 1 : -1) * (seriesValue(queueFree) + queueP1 / K);
      else if (queueP2 + queueFree == K && queueFree < 3)
        return (me == MNKCellState.P2 ? 1 : -1) * (seriesValue(queueFree) + queueP2 / K);
  ...
  public Pair<Double, MNKCell> iterativeDeepening() {
    ...
    while (!shouldHalt() && maxDepth <= len) {
      ...
      // TODO: remove in production
      if (verbose)
        System.out.println("minimax went to depth " + maxDepth + " with value: (" + value + ", " + bc + ")");
