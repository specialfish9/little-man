...
public class LittleBoy implements MNKPlayer {
  ...
  private class Board extends MNKBoard {
    ...
    private int pushCell(final MNKCellState state) {
      ...
      if (currentSeries.peekFirst() == MNKCellState.FREE)
        freeBeforeAfter *= 2;
      if (currentSeries.peekLast() == MNKCellState.FREE)
        freeBeforeAfter *= 2;
  ...
  private int pvs(int color, int searchDepth, int alpha, int beta) {
    ...
    // Check for a one-move-(win|loss) play only when enough moves have been played
      ...
      while (i < len) {
        if (i < sortUpTo)
          selectionSort(cells, ratings, i, sortUpTo, color);
        else
        ...
        if (score > alpha || score == HALT)
          alpha = score;
        if (alpha >= beta || alpha == HALT)
          break;
        i++;
      }
      result = alpha;
    ...
    if (board.gameState() == MNKGameState.OPEN) {
      if (result <= alpha)
        entry[3] = UPPER_BOUND; // store the new lower bound
      else if (result >= beta)
        entry[3] = LOWER_BOUND; // store the new upper bound
      else
        entry[3] = EXACT_VALUE; // store the exact value on a PV node
    } else {
      // if we found an ending state we can keep it with a +INFTY searchDepth as this
      // will be the definitive value for the board regardless of a greater
      // searchDepth
      entry[2] = INFTY;
  ...
  private MNKCell pvsRoot(int searchDepth, int alpha, int beta) {
    ...
    // enough cells to make one happen
      ...
      while (i < len) {
        if (i < sortUpTo)
          selectionSort(cells, ratings, i, sortUpTo, -1);
        else
          randomSelection(cells, i, len);
