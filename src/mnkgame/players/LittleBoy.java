...
public class LittleBoy implements MNKPlayer {
  ...
  private class Board extends MNKBoard {
    ...
    if(state==MNKCellState.FREE)queueFree++;else if(state==MNKCellState.P1)queueP1++;else if(state==MNKCellState.P2)queueP2++;currentSeries.add(state);

    int sign = me == MNKCellState.P1 ? 1 : -1;
    int freeBeforeAfter = 1;if(currentSeries.peekFirst()==MNKCellState.FREE)freeBeforeAfter*=2;if(currentSeries.peekLast()==MNKCellState.FREE)freeBeforeAfter*=2;
