 ...
 */
public class CharlesDarwin extends LoggedPlayer implements MNKPlayer {
  ...
  private double evaluate(final MNKBoard board, final int depth) {
    if (board.gameState() == MY_WIN) return RANK_CONSTANT / depth;
    else if (board.gameState() == ENEMY_WIN) return -(depth * RANK_CONSTANT);
  ...
  private MNKCell find_one_move_win(MinimaxBoard board, final MNKGameState win_state) {
    for (MNKCell c : board.getFreeCells()) {
      if (should_halt()) return null;
      MNKGameState result = board.markCell(c);
      board.unmarkCell();
      if (result == win_state) {
        _cutoff += board.getFreeCells().length - 1;
        return c;
      }
    }
    return null;
  ...
  private MNKCell find_one_move_loss(MinimaxBoard board, final MNKGameState loss_state) {
    ...
    if (board.getFreeCells().length == 1
        || (random_cell = pick_random_non_closing_cell(board, null)) == null)
  ...
  private Pair<Double, MNKCell> minimax(
    ...
    visited++;
    // handle the first move by placing ourselves at the center, which is the best
    // postition for any
    // mnk
    ...
    if (board.getMarkedCells().length >= (K - 1) * 2
        ...
        ((omc = find_one_move_win(board, action == Action.MAXIMIZE ? MY_WIN : ENEMY_WIN)) != null
            || (omc = find_one_move_loss(board, action == Action.MAXIMIZE ? ENEMY_WIN : MY_WIN))
                != null)) {
      ...
      if (depth == 0) return new Pair<>(0d, omc);
    ...
    for (int i = 0; i < board.getFreeCells().length; i++) {
      ...
      if ((action == Action.MAXIMIZE && (double) rank.first > best)
          || (action == Action.MINIMIZE && rank.first < best)) {
        best = rank.first;
        best_cell = c;
      }
